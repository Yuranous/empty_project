...
public class UserServiceImpl implements UserService {
    ...
    public UserView findById(Long id) throws DataNotFoundException {
        ...
        if (result.isPresent()) {
            return mapperFacade.map(result.get(), UserView.class);
        } else {
    ...
    @Transactional
    @Override
    public void update(UserUpdateView view) throws UpdateException {
        Optional<User> source = userDao.findById(view.getId());
        if (source.isPresent()) {
            User src = source.get();
            mapperFacade.map(view, src);
            Optional<Country> country = countryDao.findByCitizenshipCode(view.getCitizenshipCode());
            src.setCountry(country.orElse(null));
            Optional<DocumentType> type = documentTypeDao.findByName(view.getDocName());
            src.getDocument().setType(type.orElse(null));
            userDao.update(src);
        } else {
